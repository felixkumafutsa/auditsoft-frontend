...
const ReportsFilesPage: React.FC = () => {
    ...
    const fetchReports = async () => {
        try {
            ...
            const reportsData = data.auditReports || [];
            setAuditReports(reportsData);
            
            // TODO: Add API calls for custom reports and templates when available
        ...
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            <Paper sx={{ width: '100%', mb: 2, borderRadius: 2, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <Tabs
                    ...
                    variant="scrollable"
                    scrollButtons="auto"
                    aria-label="stored reports tabs"
                >
                    <Tab label="Generated Audit Reports" />
                    <Tab label="Custom Built Reports" />
                    <Tab label="Generated Report Templates" />
                </Tabs>
                                    ...
                                    {auditReports.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={7} align="center" sx={{ py: 3 }}>
                                                <Typography color="text.secondary">No generated audit reports found.</Typography>
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                    ...
                                    {customReports.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} align="center" sx={{ py: 3 }}>
                                                <Typography color="text.secondary">No custom built reports found.</Typography>
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        customReports.map((report) => (
                                            <TableRow hover key={report.id}>
                                                ...
                                                <TableCell>{new Date(report.createdAt).toLocaleDateString()} {new Date(report.createdAt).toLocaleTimeString()}</TableCell>
                                    ...
                                    {reportTemplates.length === 0 ? (
                                        ...
                                        reportTemplates.map((template) => (
                                            <TableRow hover key={template.id}>
                                                ...
                                                <TableCell>{new Date(template.createdAt).toLocaleDateString()} {new Date(template.createdAt).toLocaleTimeString()}</TableCell>
